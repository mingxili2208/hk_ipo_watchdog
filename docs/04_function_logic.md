...
- 默认关闭。2026-05-26 真实页面检查确认 AAStocks 暗盘实时字段由动态行情通道写入，现有静态 HTML 解析不能产生可靠报价；待独立分支完成适配与验证后启用以下逻辑。

逻辑：

```text
1. 自动调度时检查香港工作日 16:15-18:30 采集窗口；窗口外直接返回
2. 查询正在跟踪的活跃 IPO；为空时直接返回
3. collect_grey_market_quotes
4. 保存报价
5. 判断暗盘阈值
6. 按交易日、涨跌方向及显著幅度阶梯生成去重 key
7. 推送新的暗盘异动阶梯
```
...
```text
1. 按香港自然日查询今日事件，并关联当前 IPO、配发或暗盘数据快照
2. 查询此前已发现、具有有效上市日期且尚未上市的 IPO 跟踪项
3. 对跟踪项展示官方章程主营业务短摘要、计算距离上市日剩余天数，并关联首次发现日已送达的详情日报日期
4. 基于最新入库数据计算今日事件股票的当前策略评分，不额外触发单股通知
5. 构造含招股字段、主营业务短摘要、评分依据及上市倒计时的 digest payload
6. LLM 生成日报
7. 推送并保存日报记录
```
